...
const menuExpandMap = ref<Map<string | number, any>>(new Map());
const menuChildrenListMap = ref<Map<string | number, any[]>>(new Map());
...
const getChildrenList = async (row: any, treeNode: unknown, resolve: (data: any[]) => void) => {
  menuExpandMap.value.set(row.menuId, { row, treeNode, resolve });
  const children = menuChildrenListMap.value.get(row.menuId) || [];
...
const expandMenuHandle = async (row: any, expanded: boolean) => {
  if (!expanded) {
    menuExpandMap.value.delete(row.menuId);
  }
};

/** 刷新展开的菜单数据 */
const refreshLoadTree = (parentId: string | number) => {
  const expandData = menuExpandMap.value.get(parentId);
  if (expandData) {
    const { row, treeNode, resolve } = expandData;
    if (row) {
      getChildrenList(row, treeNode, resolve);
      if (row.parentId) {
        const grandpaMenu = menuExpandMap.value.get(row.parentId);
        if (grandpaMenu) {
...
const refreshAllExpandMenuData = () => {
  for (const menuId of menuExpandMap.value.keys()) {
...
const getList = async () => {
  ...
  for (const menu of res.data) {
    ...
    if (!tempMap.has(parentId)) {
      tempMap.set(parentId, []);
    }
    tempMap.get(parentId).push(menu);
    menuIdSet.add(menuId);
  }
  
  for (const menu of res.data) {
    menu['hasChildren'] = (tempMap.get(menu.menuId)?.length || 0) > 0;
  }
